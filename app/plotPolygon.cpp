...
cv::Mat LanePredictor::plotPolygon(cv::Mat inputImage, cv::Vec4d yellowLanes,
                                   cv::Vec4d whiteLanes) {
    cv::Mat output;
    ...
    cv::Point pts[4] = {lowerYellow, upperYellow, upperWhite, lowerWhite};

    std::cout << "pts" << pts[2] << "," << pts[3] << std::endl;

    cv::fillConvexPoly(output, pts, 4, cv::Scalar(0, 0, 255), CV_AA);

    line(inputImage, lowerYellow, upperYellow, cv::Scalar(25, 0, 51), 2, CV_AA);
    line(inputImage, lowerWhite, upperWhite, cv::Scalar(25, 0, 51), 2, CV_AA);
    line(inputImage, upperYellow, upperWhite, cv::Scalar(25, 0, 51), 2, CV_AA);
